-- ============================================
-- CNEC Taiwan Platform - Complete Database Schema
-- Execute this entire script in Supabase SQL Editor
-- This creates ALL tables from scratch
-- Default region: TW (Taiwan)
-- ============================================
...
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    phone_number TEXT,
    phone TEXT,
    country_code VARCHAR(2) DEFAULT 'TW' CHECK (country_code IN ('US', 'TW', 'JP', 'KR')),
    platform_region VARCHAR(10) DEFAULT 'tw' CHECK (platform_region IN ('us', 'tw', 'jp', 'kr')),
...
CREATE TABLE IF NOT EXISTS campaigns (
    ...
    requirements TEXT,
    category TEXT,
    target_country VARCHAR(2) DEFAULT 'TW' CHECK (target_country IN ('US', 'TW', 'JP', 'KR')),
    platform_region VARCHAR(10) DEFAULT 'tw' CHECK (platform_region IN ('us', 'tw', 'jp', 'kr')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
